...
export function BrandedError<T extends string>(brand: T): new (...args: any) => BaseError<T> {
...
export type ErrorResult<E extends BaseError<string>> = Readonly<[value: undefined, error: E]>;
export type Result<T, E extends BaseError<string>> = OkResult<T> | ErrorResult<E>;
...
export type MergeResults<TUnion extends OkResult<any> | ErrorResult<any>> =
  ExtractErr<TUnion> extends never
    ? Result<ExtractOk<TUnion>, never> // Solo resultados exitosos
    : ExtractOk<TUnion> extends never
    ? Result<never, ExtractErr<TUnion>> // Solo errores
...
export function tryCatch<T>(fn: () => Promise<T>): Promise<Result<T, UnknownException>>;
...
export function mapErr<T, E extends BaseError<string>, R extends BaseError<string>>(
  mapper: Mapper<E, R>,
...
>(
  ...
  return (isResult(resultOrBrandMappers) ? fn(resultOrBrandMappers) : fn) as any;
}
...
export function unwrapErr<E extends BaseError<string>>(result: ErrorResult<E>): E {
  return result[1];
}
